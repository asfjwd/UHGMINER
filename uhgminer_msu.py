...
class HyperGraph:
  ...
  # returns the msu pruning value
  def msu(self, h, last_e, phi_v, phi_e) -> float:
    utility = 0
    component = None
    
    for v in self.vertices:
      if v in phi_v:
        component = self.find_component(v)
        break
    ...
    for e in self.get_edges():
      ...
      for v in e.vertices:
        ...
        if e.id not in phi_e:
          edge_utility += vertex_utility
        else:
          if phi_e[e.id] == last_e:
            edge_utility += vertex_utility
          elif v in phi_v and h.edges[phi_e[e.id]].contains(phi_v[v]):
            edge_utility += vertex_utility
...
class Code:
  ...
  def canonical_candidate(self, t: ExtensionTuple) -> bool:
    if canonical_pruning == 'OFF':
      return True
    
    if len(self.tuples) == 0:
      return True
    
    if (self.tuples[0].edge_label, self.tuples[0].vertex_label) > (t.edge_label, t.vertex_label):
      return False
    
    if t.op == 'e':
      if t.vertex < self.tuples[-1].vertex:
        return False
      
      elif self.tuples[-1].op == 'e' and self.added_vertex[-1] == True:
        return t.vertex_label > self.tuples[-1].vertex_label
      
      elif self.tuples[-1].op == 'a':
        for i in range(len(self.tuples) - 1):
          if self.tuples[i] == self.tuples[-1]:
            if self.tuples[i + 1] > t:
              return False
            elif self.added_vertex[i + 1] == True and t.vertex > self.last_v:
              if (self.tuples[i + 1].edge_label, self.tuples[i + 1].vertex_label) > (t.edge_label, t.vertex_label):
                return False
      ...
      for tuple in self.tuples:
        if flag and tuple.op == 'a' and tuple.vertex > t.vertex:
          return False
        
        if tuple.vertex == t.vertex:
          flag = True
      ...
      for e in H.get_edges():
        for v in e.vertices:
          ...
          if canonical == False:
            ...
            pruning_value = H.msu(new_code.hypergraph, new_code.last_e, {v : 0}, {e.id : 0})
          ...
          for v in H.edges[e].vertices:
            ...
            if e not in phi_inv_e and v in phi_inv_v:
              ...
              if canonical == False:
                ...
                pruning_value = H.msu(new_code.hypergraph, new_code.last_e, phi_inv_v, new_phi_inv_e)
        ...
        for v in e.vertices:
          ...
          if v not in phi_inv_v:
            ...
            if canonical == False:
              ...
              pruning_value = H.msu(new_code.hypergraph, new_code.last_e, new_phi_inv_v, phi_inv_e)
            ...
            if canonical == False:
              ...
              pruning_value = H.msu(new_code.hypergraph, new_code.last_e, phi_inv_v, phi_inv_e)
...
def UHGMINER(code : Code, hypergraphs : list, min_util : float, lev = 0) -> None:
  global counter, candidates, not_canonical, pruned
  global file
...
def main(filename, delta):
  ...
  global file
  global counter, candidates, not_canonical, pruned
  counter, candidates, not_canonical, pruned = 0, 0, 0, 0
  if file is not None:
    file.close()
  folder = "msu_results"
